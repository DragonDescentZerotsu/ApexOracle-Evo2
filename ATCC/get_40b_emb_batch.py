...
strain_chunks_dict = chunk_fasta_seq(Path('/data2/tianang/projects/Ben_ApexOracle_test/DataPrepare/Data/RawGenome'), Path('/data2/tianang/projects/Ben_ApexOracle_test/DataPrepare/Data/GenomeEmb'))
batch_size = 3
...
with torch.no_grad():
    for strain, chunks_seqs in tqdm(strain_chunks_dict.items(), desc=' Embedding strain genome chunks... ', total=len(strain_chunks_dict)):
        ...
        torch.save(chunk_embs,
                   Path('/data2/tianang/projects/Ben_ApexOracle_test/DataPrepare/Data/GenomeEmb') / f'{strain}.pt')
        print(1)